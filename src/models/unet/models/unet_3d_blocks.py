...
class CrossAttnDownBlock3D(nn.Module):
    ...
    def forward(
        ...
        adapter_feature: Optional[torch.FloatTensor] = None,
        ...
        if adapter_feature is not None:
            if isinstance(adapter_feature, tuple):
                for feature_ in adapter_feature:
                    hidden_states += feature_
            else:
...
class CrossAttnDownBlockSpatioTemporal(nn.Module):
    ...
    def forward(
        ...
        if adapter_feature is not None:
            if isinstance(adapter_feature, tuple):
                for feature_ in adapter_feature:
                    hidden_states = hidden_states + feature_
            else:
